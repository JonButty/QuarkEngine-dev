...
namespace Descension
{
    ...
    void GCAICaster::OnAwake()
    {
        ...
        fsm_[ES_ATTACK_DEFAULT].OnEnter = Simian::Delegate::CreateDelegate<GCAICaster, &GCAICaster::attackDefaultOnEnter_>(this);
        fsm_[ES_ATTACK_DEFAULT].OnUpdate = Simian::Delegate::CreateDelegate<GCAICaster, &GCAICaster::attackDefaultOnUpdate_>(this);
	...
	void GCAICaster::idleOnUpdate_( Simian::DelegateParameter&)
	{
        if( playerAttr_ && playerAttr_->Health() > 0.0f )
        {
            updateVecToPlayer_();
            ...
            if (!TileWallCollision::CheckTileCollision(pos,vecToPlayer_*TileWallCollision::PHYSICS_SCALE,0.05f,&tMap_->TileMap(),false,false) && 
	...
	void GCAICaster::attackSkillOnUpdate_( Simian::DelegateParameter& param)
	{
        ...
        {
            if( playerAttr_ && playerAttr_->Health() < Simian::Math::EPSILON )
            {
                fsm_.ChangeState(ES_IDLE);
                return;
            }
    ...
    void GCAICaster::attackDefaultOnEnter_( Simian::DelegateParameter& )
    {
        helperStartAttack_();
    }

    void GCAICaster::attackDefaultOnUpdate_( Simian::DelegateParameter& param )
    {
        if( playerAttr_ && playerAttr_->Health() < Simian::Math::EPSILON )
        {
            fsm_.ChangeState(ES_IDLE);
    ...
    void GCAICaster::helperStartAttack_()
    {
        model_->Controller().PlayAnimation("Cast");
        attackTimer_ = 0.0f;
